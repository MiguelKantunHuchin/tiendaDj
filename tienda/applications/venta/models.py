...
class Sale(TimeStampedModel):
    ...
    state = models.CharField(
        "Estado de Envio", max_length=2, choices=FLAT_STATE, blank=True
    )
...
class SaleDetail(TimeStampedModel):
    ...
    sale = models.ForeignKey(
        Sale, on_delete=models.CASCADE, verbose_name="Codigo de Venta"
    )
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    count = models.PositiveIntegerField("Cantidad")
    price_purchase = models.DecimalField(
        "Precio Compra", max_digits=10, decimal_places=3
    )
